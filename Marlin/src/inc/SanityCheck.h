 ...
 */
    ...
    static_assert((SHAPING_ZETA_X) >= 0 && (SHAPING_ZETA_X) <= 0.99f, "SHAPING_ZETA_X must be in the range [0, 0.99].");
  #endif
  #if ENABLED(INPUT_SHAPING_Y)
    static_assert((SHAPING_FREQ_Y) > 0, "SHAPING_FREQ_Y must be > 0.");
    static_assert((SHAPING_ZETA_Y) >= 0 && (SHAPING_ZETA_Y) <= 0.99f, "SHAPING_ZETA_Y must be in the range [0, 0.99].");
  #endif
#endif
