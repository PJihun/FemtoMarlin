# Upstream Marlin Input Shaper Implementation (from stepper.cpp)

Source: https://raw.githubusercontent.com/MarlinFirmware/Marlin/main/Marlin/src/module/stepper.cpp

## How this input shaper implementation works

This upstream implementation uses a ZV-style two-pulse shaper in the stepper ISR path. For each enabled shaped axis, a move is split into:

1. A primary pulse at the original step time using `factor1`.
2. A delayed echo pulse using `factor2`.

The coefficients are fixed-point (`factor1 + factor2 = 128`) and the delay is derived from the configured shaping frequency.

## Input shaping workflow

1. Parameter setup
The host sends shaping parameters through M593 (in the G-code layer), which call `Stepper::set_shaping_frequency()` and `Stepper::set_shaping_damping_ratio()`.

2. Damping to amplitude mapping
`Stepper::set_shaping_damping_ratio()` converts damping ratio `zeta` into fixed-point amplitudes. The code uses a polynomial approximation so ISR code can use cheap integer math.

3. Frequency to delay mapping
`Stepper::set_shaping_frequency()` synchronizes motion, disables ISR briefly, computes a per-axis delay in timer ticks, updates `ShapingQueue` delay, and enables or disables shaping for that axis.

4. Primary stepping path
In `pulse_phase_isr()`, when an axis step occurs and shaping is enabled:

1. The code records an echo event in `ShapingQueue::enqueue()` with step direction.
2. It emits the first shaped contribution immediately via `PULSE_PREP_SHAPING(... factor1 ...)`.

5. Delayed echo path
`shaping_isr()` checks queues (`peek_*`, `free_count_*`) and emits delayed echo pulses using `PULSE_PREP_SHAPING(... factor2 ...)`, then toggles step pins for X/Y/Z as needed.

6. Queue safety behavior
If queue head timing is ready, or if free space gets low, echoes are applied early to avoid overflow. This trades perfect shaping fidelity for real-time safety.

7. Position and state consistency
When frequency is changed or shaping is toggled, the implementation resets shaping deltas and tracks `last_block_end_pos` so logical position and shaped step history stay coherent.

## Practical read order in this file

1. Static shaping storage and queue data under `#if HAS_ZV_SHAPING`.
2. `pulse_phase_isr()` integration point where echoes are enqueued and first pulses are applied.
3. `shaping_isr()` delayed-echo executor.
4. `set_shaping_damping_ratio()` and `set_shaping_frequency()` configuration logic.
5. `_set_position()` handling for shaped-position bookkeeping.
